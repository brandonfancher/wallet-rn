...
export default class App extends React.Component {
  ...
  createWalletFromSeed = () => {
    const { walletName, xpub } = this.state;
    console.log('trying');
    this.app.service('hd-wallet')
      .create({ name: walletName, extended_public_key: xpub })
      .then(result => console.log('Created Wallet: ', result))
      .catch(err => console.error(err));

    // this.socket.emit('hd-wallet::create', { name: walletName, extended_public_key: xpub }, (error, message) => {
    //   console.log('Created Wallet: ', message);
    // })

  }

  render() {
    const { mnemonic, mnemonicIsValid, addresses, userSeed, walletName, xpub } = this.state;
        ...
        {xpub
          ? <View>
              <TextInput
                ...
                value={walletName}
              />
              <TouchableOpacity onPress={this.createWalletFromSeed} style={buttonStyle}>
